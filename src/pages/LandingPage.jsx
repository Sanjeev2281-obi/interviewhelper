...
const LOGOS = {
 ...
 Accenture : () => (
  <svg viewBox="0 0 48 48" width="100%" height="100%">
    ...
    <text x="24" y="31" textAnchor="middle" fill="white" fontSize="14" fontWeight="900" fontFamily="Arial,sans-serif">
      ACC
    </text>
  </svg>
)
...
const FEATURES = [
  ...
  { icon: Sparkles,      color: '#a855f7', glow: 'rgba(168,85,247,0.12)', title: 'Technical Questions Bank',   desc: 'Real scenario questions — "Why didn t Hotstar crash during IPL?" OOP, DBMS, OS, Networking.', tag: 'Pro' }
]

const TESTIMONIALS = [
  { name: 'Arun Kumar',     role: 'SDE-1 @ Amazon',               avatar: 'AK', company: 'Amazon',    pkg: '₹22 LPA', text: 'Got my Amazon offer after 3 months of prep here. The company-specific patterns told me exactly what to expect in each round. The discussion community helped me understand what real engineers face.' },
  { name: 'Priya Sharma',   role: 'Software Engineer @ Walmart', avatar: 'PS', company: 'Walmart', pkg: '₹14 LPA', text: 'The resume review found 8 issues I had no idea about. After fixing them my callback rate went from 10% to 60%. The mentor guidance in Tamil made everything so much clearer for me.' },
  ...
  { name: 'Deepa Krishnan', role: 'SWE @ cognizant',                  avatar: 'DK', company: 'cognizant',    pkg: '₹7 LPA', text: 'The discussion page had a post from someone who had faced my exact Interview patterns . That single discussion saved me. The community here is genuinely helpful.' },
  { name: 'Karthik Raja',   role: 'SDE @ Flipkart',                avatar: 'KR', company: 'Flipkart',  pkg: '₹22 LPA', text: 'The 500 DSA problems with company tags are gold. I could focus only on Flipkart-tagged problems the week before my interview. Cracked it on first attempt.' },
  { name: 'Sneha Patel',    role: 'Dev @ Accenture',      avatar: 'SP', company: 'Accenture',   pkg: '₹6 LPA', text: 'Coming from a tier-3 college I thought IT job was impossible. This platform showed me the exact path. The community discussions gave me confidence that it was achievable.' },
]
...
function TestimonialScroller() {
  ...
  return (
    <div style={{ overflow: 'hidden', position: 'relative', width: '100%' }}>
      <div style={{ position:'absolute',left:0,top:0,bottom:0,width:80,background:'linear-gradient(to right,#020810,transparent)',zIndex:2,pointerEvents:'none' }}/>
      <div style={{ position:'absolute',right:0,top:0,bottom:0,width:80,background:'linear-gradient(to left,#020810,transparent)',zIndex:2,pointerEvents:'none' }}/>
      <div
        ref={trackRef}
        style={{
          display: 'flex',
      ...
      >
        {cards.map((t, i) => {
          const L = LOGOS[t.company] || LOGOS.Google
          return (
            <div key={i} style={{
              width: 320,
              flexShrink: 0,
              background: 'linear-gradient(135deg,rgba(13,27,13,0.9),rgba(10,15,26,0.9))',
              ...
              <p style={{ color:'#cbd5e1', fontSize:13, lineHeight:1.75, margin:'0 0 18px', fontStyle:'italic' }}>
                "{t.text}"
              </p>
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:38,height:38,borderRadius:12,background:'linear-gradient(135deg,#22c55e,#06b6d4)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:900,fontSize:13,color:'#fff',flexShrink:0 }}>{t.avatar}</div>
                <div style={{ flex:1 }}>
                  <p style={{ color:'#fff',fontWeight:800,fontSize:14,margin:0 }}>{t.name}</p>
                  <p style={{ color:'#475569',fontSize:12,margin:0 }}>{t.role}</p>
                </div>
                <div style={{ display:'flex',alignItems:'center',gap:6,backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,padding:'6px 10px' }}>
                  <div style={{ width:20,height:20,borderRadius:6,overflow:'hidden' }}><L/></div>
                  <span style={{ color:'#22c55e',fontWeight:900,fontSize:13 }}>{t.pkg}</span>
                </div>
              </div>
            </div>
          )
    ...
    <div style={{ backgroundColor:'#020408', color:'#fff', fontFamily:"DM Sans, system-ui, sans-serif", overflowX:'hidden' }}>
      ...
      <nav style={{ position:'fixed',top:0,left:0,right:0,zIndex:100,padding:'0 6%',height:64,display:'flex',alignItems:'center',justifyContent:'space-between',backgroundColor:scrollY>20?'rgba(2,4,8,0.92)':'transparent',backdropFilter:scrollY>20?'blur(24px)':'none',borderBottom:scrollY>20?'1px solid rgba(255,255,255,0.06)':'none',transition:'all 0.3s' }}>
        <div style={{ display:'flex',alignItems:'center',gap:10 }}>
          ...
          <span style={{ fontWeight:900,fontSize:20,letterSpacing:'-0.5px' }}>InterviewPrep</span>
        </div>
      ...
      <section style={{ position:'relative',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'120px 6% 80px',overflow:'hidden' }}>
        ...
        <div style={{ position:'relative',zIndex:1,textAlign:'center',maxWidth:900,width:'100%' }}>
          ...
          <h1 style={{ fontWeight:900,fontSize:'clamp(2.8rem,6.5vw,5.5rem)',lineHeight:1.05,letterSpacing:'-2px',margin:'0 0 24px' }}>
            ...
            <span style={{ background:'linear-gradient(135deg,#22c55e 0%,#06b6d4 45%,#a855f7 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text' }}>with Practice + Community</span>
          </h1>
          <p style={{ color:'#64748b',fontSize:'clamp(1rem,2vw,1.2rem)',maxWidth:620,margin:'0 auto 24px',lineHeight:1.75 }}>
            The complete prep platform built for Indian students: <strong style={{color:'#94a3b8'}}>500+ DSA problems</strong>, <strong style={{color:'#94a3b8'}}>resume review</strong>, <strong style={{color:'#94a3b8'}}>community discussions</strong>, and <strong style={{color:'#94a3b8'}}>1-on-1 guidance</strong>.
          </p>
              ...
              {Object.entries(LOGOS).map(([name, Logo]) => (
                <div key={name} style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:6 }}>
                  <div style={{ width:56,height:56,borderRadius:15,backgroundColor:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',padding:7,overflow:'hidden',transition:'all 0.25s',cursor:'default' }}
                    onMouseEnter={e=>{e.currentTarget.style.transform='scale(1.14) translateY(-3px)';e.currentTarget.style.borderColor='rgba(255,255,255,0.2)'}}
      ...
      {/* TESTIMONIALS */}
      <section style={{ padding:'96px 0',backgroundColor:'#020810',position:'relative',overflow:'hidden' }}>
        <div style={{ position:'absolute',inset:0,background:'radial-gradient(ellipse at 50% 0%,rgba(34,197,94,0.05),transparent 60%)',pointerEvents:'none' }}/>
        <div style={{ textAlign:'center',marginBottom:52,padding:'0 6%',position:'relative',zIndex:1 }}>
          ...
          <h2 style={{ fontWeight:900,fontSize:'clamp(2rem,4vw,3.5rem)',letterSpacing:'-1.5px',lineHeight:1.1,margin:'0 0 12px' }}>
            Students who cracked<br/><span style={{color:'#22c55e'}}>their dream companies</span>
          </h2>
          <p style={{ color:'#334155',fontSize:15,margin:0 }}>Hover to pause · Stories from real students</p>
        </div>
        <TestimonialScroller/>
      </section>

      {/* COMMUNITY */}
      <section id="community" style={{ padding:'96px 6%',backgroundColor:'#020408',position:'relative',overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1100,margin:'0 auto',position:'relative',zIndex:1 }}>
          <div style={{ textAlign:'center',marginBottom:56 }}>
            ...
            <p style={{ color:'#334155',fontSize:16,maxWidth:520,margin:'0 auto' }}>A community of students who've walked the same path — discussing, sharing, and helping each other crack their dream companies.</p>
          </div>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))',gap:16,marginBottom:20 }}>
            ...
            <div style={{ background:'linear-gradient(135deg,rgba(16,185,129,0.06),rgba(6,182,212,0.06))',border:'1px solid rgba(16,185,129,0.2)',borderRadius:20,padding:28 }}>
              <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:18 }}>
                <div style={{ width:44,height:44,borderRadius:14,background:'rgba(16,185,129,0.15)',display:'flex',alignItems:'center',justifyContent:'center' }}><MessageCircle size={22} color="#10b981"/></div>
                <div>
              ...
              <p style={{ fontSize:13,color:'#9ca3af',lineHeight:1.7,marginBottom:16 }}>Real interview experiences from students who just went through the process. Ask about DSA, interview tips, company patterns, salary negotiation — anything.</p>
            ...
            <div style={{ background:'linear-gradient(135deg,rgba(245,158,11,0.06),rgba(251,191,36,0.04))',border:'1px solid rgba(245,158,11,0.2)',borderRadius:20,padding:28 }}>
              <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:18 }}>
                <div style={{ width:44,height:44,borderRadius:14,background:'rgba(245,158,11,0.15)',display:'flex',alignItems:'center',justifyContent:'center' }}><UserCheck size={22} color="#f59e0b"/></div>
                <div>
              ...
              <div style={{ display:'flex',alignItems:'center',gap:12,marginBottom:14,background:'rgba(0,0,0,0.3)',borderRadius:12,padding:'12px 14px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:12,color:'#10b981' }}>SDE · Freelancer · Building SaaS products</div>
              ...
              <p style={{ fontSize:13,color:'#9ca3af',lineHeight:1.7,marginBottom:14 }}>A fellow student who has deeply explored the SDE world — researched interview patterns, career paths, and what actually works. Ask anything in English or Tamil.</p>
              <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
                ...
                <div style={{ fontSize:11,color:'#4b5563',textAlign:'center',marginTop:2 }}>+2 more topics · WhatsApp access for Pro members</div>
              </div>
            </div>
            {/* Senior SDE insights card */}
            <div style={{ background:'linear-gradient(135deg,rgba(168,85,247,0.06),rgba(59,130,246,0.06))',border:'1px solid rgba(168,85,247,0.2)',borderRadius:20,padding:28 }}>
              <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:18 }}>
                <div style={{ width:44,height:44,borderRadius:14,background:'rgba(168,85,247,0.15)',display:'flex',alignItems:'center',justifyContent:'center' }}><Sparkles size={22} color="#a855f7"/></div>
                <div>
              ...
              <p style={{ fontSize:13,color:'#9ca3af',lineHeight:1.7,marginBottom:16 }}>Senior engineers from Google, Amazon, Microsoft share insights on what they look for in candidates, how teams actually work, and career growth tips.</p>
              {[
      ...
      {/* COMPANIES */}
      <section id="companies" style={{ padding:'96px 6%',backgroundColor:'#020810',position:'relative',overflow:'hidden' }}>
        <div style={{ maxWidth:1100,margin:'0 auto',position:'relative',zIndex:1 }}>
          <div style={{ textAlign:'center',marginBottom:56 }}>
            ...
            {COMPANIES.map(({name,pkg,rounds,accent})=>{
              ...
              return (
                <div key={name} style={{ backgroundColor:'#0d1117',border:'1px solid rgba(255,255,255,0.06)',borderRadius:18,padding:'22px 16px',textAlign:'center',transition:'all 0.25s',cursor:'default' }}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=accent+'55';e.currentTarget.style.transform='translateY(-5px)'}}
      ...
      {/* FEATURES */}
      <section id="features" style={{ padding:'96px 6%',backgroundColor:'#020408' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:56 }}>
            ...
            <h2 style={{ fontWeight:900,fontSize:'clamp(2rem,4vw,3.5rem)',letterSpacing:'-1.5px',lineHeight:1.1,margin:'0 0 14px' }}>7 tools. 1 subscription.<br/><span style={{color:'#a855f7'}}>Everything you need.</span></h2>
          </div>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:14 }}>
            {FEATURES.map(({icon:Icon,color,glow,title,desc,tag})=>(
              <div key={title} style={{ backgroundColor:'#0d1117',border:'1px solid rgba(255,255,255,0.06)',borderRadius:20,padding:'28px',position:'relative',overflow:'hidden',transition:'all 0.3s',cursor:'default' }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor=color+'40';e.currentTarget.style.transform='translateY(-5px)'}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.06)';e.currentTarget.style.transform='translateY(0)'}}>
      ...
      {/* PRICING */}
      <section id="pricing" style={{ padding:'96px 6%',position:'relative',overflow:'hidden',backgroundColor:'#020810' }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:20 }} className="pricing-grid">
            <div style={{ backgroundColor:'#0d1117',border:'1px solid rgba(255,255,255,0.08)',borderRadius:24,padding:36,display:'flex',flexDirection:'column' }}>
              ...
              <ul style={{ listStyle:'none',padding:0,margin:'0 0 32px',flex:1 }}>
                {['25 DSA problems','Progress tracking','1 System Design problem','Community discussions','Basic company patterns'].map(f=>(
            ...
            <div style={{ background:'linear-gradient(160deg,#0a1f10,#071a22)',border:'2px solid rgba(34,197,94,0.45)',borderRadius:24,padding:36,display:'flex',flexDirection:'column',position:'relative',overflow:'hidden' }}>
              ...
              <p style={{ color:'#4ade80',fontSize:11,fontWeight:800,letterSpacing:3,textTransform:'uppercase',margin:'16px 0 12px' }}>Pro Plan</p>
              <div style={{ marginBottom:24 }}>
                ...
                <p style={{ color:'#334155',fontSize:12,margin:'4px 0 0' }}>Per month · Cancel anytime</p>
              </div>
              ...
              <Link to="/signup" style={{ display:'block',textAlign:'center',padding:'15px',background:'linear-gradient(135deg,#22c55e,#16a34a)',color:'#fff',fontWeight:900,fontSize:15,borderRadius:14,textDecoration:'none',boxShadow:'0 0 30px rgba(34,197,94,0.3)',transition:'all 0.2s' }} onMouseEnter={e=>{e.currentTarget.style.transform='scale(1.02)'}} onMouseLeave={e=>{e.currentTarget.style.transform='scale(1)'}}>
                Start Pro — ₹299/month
              </Link>
            </div>
          </div>
      ...
      {/* CTA */}
      <section style={{ padding:'96px 6%',textAlign:'center',position:'relative',overflow:'hidden',backgroundColor:'#020408' }}>
        ...
        <div style={{ maxWidth:680,margin:'0 auto',position:'relative',zIndex:1 }}>
          ...
          <p style={{ color:'#334155',fontSize:17,margin:'0 0 36px' }}>Join 10,000+ students who cracked Google, Amazon, Microsoft and more.</p>
      ...
      <footer style={{ borderTop:'1px solid rgba(255,255,255,0.05)',backgroundColor:'#020408',padding:'28px 6%' }}>
        <div style={{ maxWidth:1100,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:14 }}>
          <div style={{ display:'flex',alignItems:'center',gap:10 }}>
            ...
            <span style={{ fontWeight:900,fontSize:18,color:'#fff' }}>InterviewPrep</span>
          </div>
          <p style={{ color:'#1e293b',fontSize:13 }}>2026 InterviewPrep · Built for Indian tech students</p>
