...
import { ArrowRight, Star, CheckCircle, Zap, Github, Linkedin, Twitter, Code2, Brain, FileText, Building2, Target, TrendingUp, Play, Crown, Users, Award } from 'lucide-react'
...
const FEATURES = [
  ...
  { icon: Brain,      color: '#a855f7', glow: 'rgba(168,85,247,0.12)', title: 'AI Mock Interviews',         desc: 'Simulate real conditions with AI. Instant feedback on your answers, pacing, and communication.', tag: 'AI' },
  { icon: FileText,   color: '#06b6d4', glow: 'rgba(6,182,212,0.12)',  title: 'Resume Review & Score',      desc: 'Upload your resume. Get a score out of 100 with specific, actionable points to improve it.', tag: 'Pro' },
...
const TESTIMONIALS = [
  { name: 'Arun Kumar',     role: 'SDE-1 @ Amazon',               avatar: 'AK', company: 'Amazon',    pkg: 'Rs 32 LPA', text: 'Got my Amazon offer after 3 months of prep here. The company-specific patterns told me exactly what to expect in each round. AI mock interviews built my confidence tremendously.' },
  { name: 'Priya Sharma',   role: 'Software Engineer @ Microsoft', avatar: 'PS', company: 'Microsoft', pkg: 'Rs 28 LPA', text: 'The AI resume review found 8 issues I had no idea about. After fixing them my callback rate went from 10% to 60%. Got Microsoft and two other offers in the same week.' },
  { name: 'Rohit Nair',     role: 'Backend Dev @ Zoho',            avatar: 'RN', company: 'Zoho',      pkg: 'Rs 10 LPA', text: 'First attempt at Zoho and I cracked it. The round-by-round breakdown was surgical — I knew exactly what topics each round would cover. Felt like I had insider information.' },
  { name: 'Deepa Krishnan', role: 'SWE @ Google',                  avatar: 'DK', company: 'Google',    pkg: 'Rs 55 LPA', text: 'The System Design section for Google is genuinely elite. The HLD approach for Google-specific problems helped me nail the system design round that most candidates fail.' },
]

const COMPANIES = [
  { name: 'Google',    pkg: 'Rs 35-80 LPA', rounds: '5 rounds', accent: '#4285F4' },
  { name: 'Amazon',    pkg: 'Rs 20-45 LPA', rounds: '4 rounds', accent: '#FF9900' },
  { name: 'Meta',      pkg: 'Rs 30-60 LPA', rounds: '5 rounds', accent: '#0866FF' },
  { name: 'Apple',     pkg: 'Rs 30-70 LPA', rounds: '5 rounds', accent: '#A2AAAD' },
  { name: 'Netflix',   pkg: 'Rs 40-90 LPA', rounds: '5 rounds', accent: '#E50914' },
  { name: 'Microsoft', pkg: 'Rs 25-55 LPA', rounds: '4 rounds', accent: '#00A4EF' },
  { name: 'Zoho',      pkg: 'Rs 5-12 LPA',  rounds: '4 rounds', accent: '#E42527' },
  { name: 'Flipkart',  pkg: 'Rs 18-40 LPA', rounds: '4 rounds', accent: '#2874F0' },
  { name: 'Walmart',   pkg: 'Rs 15-35 LPA', rounds: '4 rounds', accent: '#0071CE' },
  { name: 'Uber',      pkg: 'Rs 20-50 LPA', rounds: '4 rounds', accent: '#666' },
]
  ...
  return (
    <div style={{ backgroundColor: '#020408', color: '#fff', fontFamily: "DM Sans, system-ui, sans-serif", overflowX: 'hidden' }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '120px 6% 80px', overflow: 'hidden' }}>
        ...
        <div style={{ position:'relative',zIndex:1,textAlign:'center',maxWidth:900,width:'100%' }}>
          ...
          <h1 style={{ fontWeight:900,fontSize:'clamp(2.8rem,6.5vw,5.5rem)',lineHeight:1.05,letterSpacing:'-2px',margin:'0 0 24px' }}>
            <span style={{ color:'#fff' }}>Land Your Dream Job</span><br/>
            <span style={{ background:'linear-gradient(135deg,#22c55e 0%,#06b6d4 45%,#a855f7 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text' }}>at FAANG with AI</span>
          </h1>

          <p style={{ color:'#64748b',fontSize:'clamp(1rem,2vw,1.2rem)',maxWidth:600,margin:'0 auto 48px',lineHeight:1.75 }}>
            The complete prep platform: <strong style={{color:'#94a3b8'}}>500+ DSA problems</strong>, <strong style={{color:'#94a3b8'}}>AI mock interviews</strong>, <strong style={{color:'#94a3b8'}}>resume review</strong>, and <strong style={{color:'#94a3b8'}}>system design</strong>.
          </p>
      ...
      <div style={{ borderTop:'1px solid rgba(255,255,255,0.05)',borderBottom:'1px solid rgba(255,255,255,0.05)',backgroundColor:'rgba(255,255,255,0.015)',padding:'44px 6%' }}>
        <div style={{ maxWidth:800,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:8 }}>
          {[{icon:Users,value:10000,suffix:'+',label:'Students Placed',color:'#22c55e'},{icon:Code2,value:500,suffix:'+',label:'DSA Problems',color:'#06b6d4'},{icon:Building2,value:10,suffix:'',label:'Top Companies',color:'#a855f7'},{icon:Award,value:4.9,suffix:'*',label:'Avg Rating',color:'#f59e0b'}].map(({icon:Icon,value,suffix,label,color})=>(
      ...
      <section id="companies" style={{ padding:'96px 6%',backgroundColor:'#020810',position:'relative',overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1100,margin:'0 auto',position:'relative',zIndex:1 }}>
          <div style={{ textAlign:'center',marginBottom:56 }}>
            ...
            <p style={{ color:'#334155',fontSize:16,maxWidth:480,margin:'0 auto' }}>Real interview patterns. Round-by-round breakdown. Insider tips.</p>
          </div>
      ...
      <section id="features" style={{ padding:'96px 6%' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:56 }}>
            ...
            <h2 style={{ fontWeight:900,fontSize:'clamp(2rem,4vw,3.5rem)',letterSpacing:'-1.5px',lineHeight:1.1,margin:'0 0 14px' }}>6 tools. 1 subscription.<br/><span style={{color:'#a855f7'}}>Everything you need.</span></h2>
          </div>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:14 }}>
            {FEATURES.map(({icon:Icon,color,glow,title,desc,tag})=>(
              <div key={title} style={{ backgroundColor:'#0d1117',border:'1px solid rgba(255,255,255,0.06)',borderRadius:20,padding:'28px',position:'relative',overflow:'hidden',transition:'all 0.3s',cursor:'default' }}
                ...
                <h3 style={{ color:'#fff',fontWeight:800,fontSize:18,margin:'0 0 10px',position:'relative' }}>{title}</h3>
                <p style={{ color:'#475569',fontSize:14,lineHeight:1.7,margin:0,position:'relative' }}>{desc}</p>
              </div>
            ))}
          </div>
        </div>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ display:'flex',alignItems:'center',gap:14,flexWrap:'wrap' }}>
                  ...
                  <div style={{ display:'flex',alignItems:'center',gap:10,backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:'8px 14px' }}>
                    <div style={{ width:26,height:26,borderRadius:8,overflow:'hidden' }}>{TestiLogo&&<TestiLogo/>}</div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:20 }} className="pricing-grid">
            <div style={{ backgroundColor:'#0d1117',border:'1px solid rgba(255,255,255,0.08)',borderRadius:24,padding:36,display:'flex',flexDirection:'column' }}>
              ...
              <div style={{ marginBottom:32 }}>
                <span style={{ fontSize:64,fontWeight:900,color:'#fff',letterSpacing:'-3px' }}>Rs 0</span>
                <p style={{ color:'#1e3a4a',fontSize:13,margin:'4px 0 0' }}>Forever free - No card needed</p>
              </div>
              <ul style={{ listStyle:'none',padding:0,margin:'0 0 32px',flex:1 }}>
                {['20 DSA problems','3 mock interviews/month','Progress tracking','1 System Design problem','Community access'].map(f=>(
            ...
            <div style={{ background:'linear-gradient(160deg,#0a1f10,#071a22)',border:'2px solid rgba(34,197,94,0.45)',borderRadius:24,padding:36,display:'flex',flexDirection:'column',position:'relative',overflow:'hidden' }}>
              ...
              <p style={{ color:'#4ade80',fontSize:11,fontWeight:800,letterSpacing:3,textTransform:'uppercase',margin:'16px 0 20px',position:'relative' }}>Pro Plan</p>
              <div style={{ marginBottom:32,position:'relative' }}>
                <span style={{ fontSize:64,fontWeight:900,color:'#fff',letterSpacing:'-3px' }}>Rs 499</span>
                <span style={{ fontSize:20,color:'#334155',fontWeight:600 }}>/month</span>
                <p style={{ color:'#1e3a4a',fontSize:13,margin:'4px 0 0' }}>Cancel anytime - No hidden fees</p>
              </div>
              <ul style={{ listStyle:'none',padding:0,margin:'0 0 32px',flex:1,position:'relative' }}>
                {['500+ DSA problems (all companies)','Unlimited mock interviews','AI resume review and scoring','AI interview practice chat','All 8 company patterns','30 System Design problems','Priority support','Cloud progress sync'].map(f=>(
                  <li key={f} style={{ display:'flex',alignItems:'center',gap:12,padding:'9px 0',borderBottom:'1px solid rgba(255,255,255,0.04)',color:'#e2e8f0',fontSize:14 }}>
                    <div style={{ width:20,height:20,borderRadius:'50%',backgroundColor:'rgba(34,197,94,0.2)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}><CheckCircle size={11} color="#4ade80"/></div>
                    {f}
                  </li>
                ))}
              </ul>
              <Link to="/signup" style={{ display:'block',textAlign:'center',padding:'16px',background:'linear-gradient(135deg,#22c55e,#16a34a)',color:'#fff',fontWeight:900,fontSize:15,borderRadius:14,textDecoration:'none',boxShadow:'0 0 30px rgba(34,197,94,0.3)',transition:'all 0.2s',position:'relative' }} onMouseEnter={e=>{e.currentTarget.style.transform='scale(1.02)';e.currentTarget.style.boxShadow='0 0 50px rgba(34,197,94,0.5)'}} onMouseLeave={e=>{e.currentTarget.style.transform='scale(1)';e.currentTarget.style.boxShadow='0 0 30px rgba(34,197,94,0.3)'}}>
                Start Pro - Rs 499/month
              </Link>
            </div>
          </div>

          <div style={{ marginTop:20,backgroundColor:'rgba(34,197,94,0.04)',border:'1px solid rgba(34,197,94,0.12)',borderRadius:18,padding:'18px 28px',textAlign:'center' }}>
            <p style={{ color:'#64748b',fontSize:14,margin:0 }}>Think about it: Rs 499/month = Rs 16/day. One job offer = <strong style={{color:'#22c55e'}}>Rs 10-30 LPA salary jump</strong>. The ROI is <strong style={{color:'#22c55e'}}>1000x+</strong>.</p>
          </div>
        </div>
      </section>
